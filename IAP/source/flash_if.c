...
void FLASH_if_EraseApp(void)
{
	...
	startaddr = FLASH_APP_START_ADDR;
	FLASH_if_Init();
	do
	{
		if (FLASH_if_ErasePage(startaddr) == FALSE)
			break;
		startaddr += FLASH_PAGE_BYTES;
	}while(startaddr < FLASH_APP_END_ADDR);
}
void FLASH_if_IAPEnd(u16 flag)
{
	FLASH_if_Init();
	FLASH_if_ErasePage(BOOT_FLAG_ADDR);
	FLASH_if_Writehalfword(BOOT_FLAG_ADDR,flag);
	FLASH_if_Lock();
}
